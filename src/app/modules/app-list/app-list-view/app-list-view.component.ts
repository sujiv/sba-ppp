...
export class AppListViewComponent implements OnInit {
  ...
  displayedColumns: string[] = ['name', 'tin', 'account', 'irs941', 'payrollCostReport', 'healthcareCostReport', 'status'];
  dataSource: MatTableDataSource<PPPApplicationList>;
  ...
  ngOnInit() {
    this.dataSource = new MatTableDataSource<PPPApplicationList>();
    this.reviewService.getAllAppDetailsOb().subscribe(list => {
      console.log(list);
      for (const ppp of list) {
        this.dataSource.data.push(ppp);
      }
